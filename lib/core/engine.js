...
class Engine {
  constructor(options) {
    try {
      options.preScriptObj = loadPrePostScripts(options.preScript);
  ...
  run(url, scriptsByCategory, asyncScriptsByCategory) {
    ...
    if (options.preURL) {
      options.preScriptObj.push(preURL);
    }

    if (options.speedIndex || options.video) {
      options.preScriptObj.push(startVideo);
    ...
    function runIteration(index) {
      ...
      return Promise.resolve({
        ...
        .tap(() => runner.start())
        .tap(() => Promise.mapSeries(options.preScriptObj, (preScriptObj) => preScriptObj.run(
          {
            url,
            options,
            log,
